      ...
      >
        ...
        <div className="flex justify-between items-center px-6 py-4 border-b border-slate-200 dark:border-slate-800 shrink-0">
          <div>
            <h2 className="text-base font-bold text-slate-900 dark:text-slate-100">
              {initialData ? 'تعديل القيد' : (type === 'revenue' ? 'تسجيل إيراد جديد (توريد نقدية)' : 'تسجيل مصروف جديد (صرف نقدية)')}
            </h2>
            <p className="text-xs text-slate-500 mt-0.5">تسجيل حركة الأموال في خزينة السنتر ومتابعة البنود والمصروفات</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
          ...
          <div className="p-4 bg-slate-50 dark:bg-slate-900/90 border-t border-slate-200 dark:border-slate-800 flex justify-end gap-2.5 shrink-0">
            ...
            <button 
              type="submit" 
              className={`px-5 py-2 text-white rounded-lg text-xs font-bold transition-colors shadow-xs cursor-pointer ${
