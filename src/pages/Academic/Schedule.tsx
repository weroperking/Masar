...
export function Schedule() {
  ...
  const { data: allEnrollments = [] } = useApiQuery<Enrollment>(
    "enrollments",
    60 * 1000,
  );
  ...
  const uniqueRooms = Array.from(
    new Set(groups?.map((g) => g.room).filter(Boolean)),
  ) as string[];
  ...
  // KPIs
  const groupsToday = filteredGroups.filter((g) =>
    g.daysOfWeek?.includes(todayArabic),
  );
    ...
    createSession.mutate(
      ...
      },
      {
        onSuccess: () => {
          toast.success(
            isTrial
              ? "تم بدء الحصة التجريبية. انتقل لصفحة الحضور لإدارتها."
              : "تم بدء الحصة. انتقل لصفحة الحضور لإدارتها.",
          );
        },
      },
    );
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">
            جدول الحصص والمواعيد
          </h1>
          <p className="text-sm text-slate-500 mt-1">
            عرض وتنظيم مواعيد المجموعات والحصص
          </p>
        </div>
      ...
      {view === "week" && (
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-100 dark:border-slate-800 p-6">
          ...
          <div className="flex flex-col gap-6">
            {arabicDays.map((day) => {
              const dayGroups =
                filteredGroups.filter((g) => g.daysOfWeek?.includes(day)) || [];
              ...
              return (
                ...
                >
                  <div className="flex flex-col sm:w-48 shrink-0 sm:border-l border-slate-200 dark:border-slate-700 sm:pl-4 justify-center items-start sm:items-center">
                    <span
                      className={`font-bold text-sm ${isToday ? "text-blue-600" : "text-slate-800 dark:text-slate-200"}`}
                    >
                      {day}
                    </span>
                    {isToday && (
                          ...
                          >
                            <div>
                              <div className="flex justify-between items-start">
                                <span
                                  className="font-bold text-slate-900 dark:text-slate-100 text-xs leading-snug cursor-pointer hover:text-blue-600"
                                  onClick={() =>
                              ...
                              <p
                                className="text-slate-500 mt-1 font-mono"
                                dir="rtl"
                              >
                                {formatTimeRange12(
                                  group.startTime,
                              ...
                              {group.room && (
                                <p className="text-slate-500 text-[10px] mt-1 bg-slate-100 dark:bg-slate-800 inline-block px-1.5 py-0.5 rounded">
                                  قاعة: {group.room}
                                </p>
                              )}
                            </div>
                            ...
                            <div className="pt-2 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                              ...
                              {!isLive ? (
                                <button
                                  ...
                                  className="text-emerald-600 hover:text-emerald-700 bg-emerald-50 dark:bg-emerald-900/30 px-2 py-1 rounded font-bold flex items-center text-[10px] transition-colors"
                                  title="بدء الحصة"
                                >
      ...
      {view === "day" && (
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-100 dark:border-slate-800 p-6 space-y-6">
          ...
          <div>
            ...
            {filteredGroups.filter((g) => g.daysOfWeek?.includes(selectedDay))
              .length === 0 ? (
              <div className="py-16 text-center text-slate-400">
                لا توجد مجموعات مجدولة في يوم {selectedDay}
              </div>
            ) : (
              <div className="space-y-3">
                {filteredGroups
                  .filter((g) => g.daysOfWeek?.includes(selectedDay))
                  .map((group) => {
                    const isLive = activeSessions?.some(
                      (s) => s.groupId === group.id,
                    );
                    return (
                      <div
                        key={group.id}
                        className="p-4 border border-slate-200 dark:border-slate-700 rounded-xl flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors"
                      >
                        <div className="flex items-start gap-4">
                          <div
                            className="text-blue-600 dark:text-blue-400 font-mono font-bold text-base mt-0.5"
                            dir="rtl"
                          >
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-500 mt-1">
                              ينتهي في {formatTime12(group.endTime)} • السعة:{" "}
                              {group.maxStudents || "مفتوح"} طالب{" "}
                              {group.room && `• قاعة: ${group.room}`}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-3 self-end md:self-auto">
                          ...
                          {!isLive ? (
                            <button
                              onClick={() =>
                                handleStartSession(group.id, group.courseId)
                              }
